...
- Админам при каждом сообщении пользователя показывается полная информация о клиенте из Remnawave API:
  - Имя/описание клиента
  - Email (если указан)
  - Username в панели
  - Статус подписки (ACTIVE/EXPIRED и т.д.)
...
В `bot/services/remnawave.py` используется эндпоинт `GET /api/users/by-telegram-id/{telegramId}`.

**Поддерживаемые форматы ответа:**
- Прямой массив: `[{...}]`
- Обертка с response: `{"response": [{...}]}`
- Один объект: `{...}`

**Используемые поля из API:**
- `status` — статус подписки (ACTIVE, EXPIRED, SUSPENDED и т.д.)
- `expireAt` — дата истечения подписки (ISO 8601)
- `subscriptionUrl` — ссылка на подписку
- `userTraffic.usedTrafficBytes` — использованный трафик в байтах
- `id` — ID пользователя в панели
- `username` — username пользователя в панели
- `description` — описание/имя пользователя
- `email` — email пользователя

При другой схеме API измените клиент в `bot/services/remnawave.py` и/или `.env`.
